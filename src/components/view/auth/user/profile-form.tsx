    ...
    <>
      <form action={handleSubmit} className="my-8 grid grid-cols-2 gap-10 overflow-auto rounded-2xl border-2 bg-white p-12">
        <div className="col-span-2 flex items-center gap-5">
          <img src={profile?.img} alt="Card" className="h-48 w-48 rounded-full" width={200} height={200} />
          <label
            htmlFor="upload"
        ...
        <div>
          ...
          <Input defaultValue={data?.email} name="email" className="rounded-2xl border-2 px-5 py-9 text-xl" placeholder="البريد الألكتروني" />
        </div>
        <div className="relative">
          <p className="py-4 text-xl font-semibold">رقم الجوال</p>
          <Input defaultValue={data?.phone} className="rounded-2xl border-2 px-5 py-9 text-xl" name="phone" placeholder="رقم الجوال" />
